...
/// <summary>Verifies the remappable global-hotkey parser.</summary>
public class HotkeyGestureTests
{
    ...
    [InlineData("")]
    [InlineData(null)]
    // Empty tokens: these threw IndexOutOfRangeException out of a method whose whole contract is
    // to return false for anything it cannot parse. The value comes from the registry, so a
    // hand-edited hotkey was enough to produce them.
    [InlineData("Win++A")]         // empty token between separators
    [InlineData("Win+A+")]         // trailing separator
