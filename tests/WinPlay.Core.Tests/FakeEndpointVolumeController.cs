...
internal sealed class FakeEndpointVolumeController : IEndpointVolumeController
{
    ...
    /// <summary>Which device the guardian asked to watch; null when it is not watching.</summary>
    public string? Watching { get; private set; }

    public void Watch(string? deviceId) => Watching = deviceId;

    /// <summary>Simulates the system (or the user's volume keys) changing the endpoint.</summary>
    public void RaiseExternalChange(bool muted, float volume)
    {
        string? id = Watching ?? DefaultRenderDeviceId;
        if (id is not null && _devices.TryGetValue(id, out var e)) { e.Mute = muted; e.Volume = volume; }
        ExternalChange?.Invoke(muted, volume);
    }
