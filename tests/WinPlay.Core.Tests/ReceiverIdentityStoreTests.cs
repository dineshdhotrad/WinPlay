...
/// <summary>
/// Verifies receiver identity pinning: a substituted device is detected and refused,
...
public sealed class ReceiverIdentityStoreTests : IDisposable
{
    ...
    {
        var check = NewStore().Check("AABBCC000001", HomePodKey);
    ...
    {
        var store = NewStore();
        store.Pin("AABBCC000001", HomePodKey, "Bedroom");

        var check = store.Check("AABBCC000001", HomePodKey);
    ...
    public void A_Substituted_Device_Is_Detected_And_Not_Acceptable()
    {
        ...
        var check = store.Check("AABBCC000001", ImpostorKey);
    ...
    {
        NewStore().Pin("AABBCC000001", HomePodKey, "Bedroom");

        // A completely new instance — as after an app restart — must still enforce the pin.
        var check = NewStore().Check("AABBCC000001", ImpostorKey);
        Assert.Equal(IdentityTrust.Mismatch, check.Trust);
    }
    ...
    {
        var store = NewStore();
        store.Pin("AABBCC000001", HomePodKey);
        Assert.Equal(IdentityTrust.Mismatch, store.Check("AABBCC000001", ImpostorKey).Trust);

        Assert.True(store.Forget("AABBCC000001"));

        Assert.Equal(IdentityTrust.FirstUse, store.Check("AABBCC000001", ImpostorKey).Trust);
        Assert.False(store.Forget("AABBCC000001")); // already gone
    }
    ...
    {
        var store = NewStore();
        store.Pin("AA:BB:CC:00:00:01", HomePodKey.ToLowerInvariant());

        Assert.Equal(IdentityTrust.Trusted, store.Check("aabbcc000001", HomePodKey).Trust);
        Assert.Equal(IdentityTrust.Trusted, store.Check("AA-BB-CC-00-00-01", HomePodKey.ToLowerInvariant()).Trust);
    }
    ...
    {
        var store = NewStore();
        store.Pin("DEV1", HomePodKey, "Bedroom");
        ...
        Assert.Equal("Bedroom", entry.Name);
    ...
    public void Multiple_Receivers_Are_Pinned_Independently()
    {
        var store = NewStore();
        store.Pin("DEV1", HomePodKey, "Bedroom");
        store.Pin("DEV2", ImpostorKey, "Office");
