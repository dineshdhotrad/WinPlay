...
/// <summary>
/// Covers the sender-side DACP endpoint: request parsing for every command a receiver
...
public class DacpTests
{
    ...
    public async Task A_Valid_Command_Over_A_Real_Socket_Is_Accepted_And_Raised()
    {
        ...
        server.CommandReceived += (_, cmd) => { received = cmd; gate.Release(); };
        server.Start();
    ...
    public async Task A_Wrong_Active_Remote_Is_Rejected_With_403_And_Raises_Nothing()
    {
        ...
        bool raised = false;
        server.CommandReceived += (_, _) => raised = true;
        server.Start();
    ...
    public async Task A_Volume_Request_Over_A_Real_Socket_Raises_The_Requested_Db()
    {
        ...
        double? db = null;
        using var gate = new SemaphoreSlim(0, 1);
        server.VolumeRequested += (_, v) => { db = v; gate.Release(); };
        server.Start();
    ...
    [Fact]
    public async Task A_Per_Destination_Token_Identifies_Which_Receiver_Asked()
    {
        // The whole point of issuing a token per destination: a volume change from one speaker
        // must be attributable to that speaker. Without it every request looked identical and the
        // app could only apply volume to everything, so turning down a HomePod also turned down
        // an unrelated room the user was streaming to at the same time.
        using var server = new DacpServer(activeRemote: "shared");
        string kitchen = server.IssueToken("kitchen");
        string study = server.IssueToken("study");

        var seen = new List<(string Origin, double Db)>();
        using var gate = new SemaphoreSlim(0, 2);
        server.VolumeRequested += (origin, v) => { lock (seen) seen.Add((origin, v)); gate.Release(); };
        server.Start();

        await SendAsync(server.Port, Request("/ctrl-int/1/setproperty?dmcp.device-volume=-12", kitchen));
        await SendAsync(server.Port, Request("/ctrl-int/1/setproperty?dmcp.device-volume=-3", study));

        Assert.True(await gate.WaitAsync(TimeSpan.FromSeconds(5)));
        Assert.True(await gate.WaitAsync(TimeSpan.FromSeconds(5)));
        lock (seen)
        {
            Assert.Contains(seen, x => x.Origin == "kitchen" && Math.Abs(x.Db + 12) < 0.01);
            Assert.Contains(seen, x => x.Origin == "study" && Math.Abs(x.Db + 3) < 0.01);
        }
    }

    [Fact]
    public async Task A_Revoked_Token_Stops_Being_Accepted()
    {
        // Tokens outlive nothing: when a destination stops, its token must stop working, or a
        // receiver that was disconnected could still drive the app.
        using var server = new DacpServer(activeRemote: "shared");
        string token = server.IssueToken("patio");
        server.RevokeToken(token);
        server.Start();

        string status = await SendAsync(server.Port, Request("/ctrl-int/1/playpause", token));
        Assert.Contains("403", status);
    }
...
/// <summary>
/// Covers the DNS-SD responder that makes the DACP endpoint discoverable: which
...
public class MdnsServiceAdvertiserTests
{
    private sealed class FakeTransport : IMdnsTransport
    {
        ...
        public IReadOnlyList<IPAddress> LocalAddresses { get; set; } = [IPAddress.Parse("10.0.0.162")];
        public void Start() { }
    ...
    public void Start_Announces_The_Full_Record_Set()
    {
        ...
        Assert.Equal(IPAddress.Parse("10.0.0.162"),
    ...
    public void A_Matching_Query_Is_Answered()
    {
        ...
        transport.Deliver(Query("_dacp._tcp.local", DnsType.Ptr), new IPEndPoint(IPAddress.Parse("10.0.0.3"), 5353));
    ...
    public void A_Unicast_Question_Is_Answered_Directly_To_The_Asker()
    {
        ...
        var from = new IPEndPoint(IPAddress.Parse("10.0.0.3"), 5353);
    ...
    public void A_Query_That_Already_Knows_Us_Is_Suppressed()
    {
        ...
        transport.Deliver(query, new IPEndPoint(IPAddress.Parse("10.0.0.3"), 5353));
