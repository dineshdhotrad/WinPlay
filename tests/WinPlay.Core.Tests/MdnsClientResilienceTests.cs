...
/// property under test is a property of the loop as it actually runs.</para>
/// </summary>
public class MdnsClientResilienceTests
{
    /// <summary>Sends one datagram to the mDNS port on loopback, as a device on the LAN would.</summary>
    private static void SendToMdnsPort(byte[] payload)
    {
        using var sender = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
        sender.SendTo(payload, new IPEndPoint(IPAddress.Loopback, 5353));
    }
    ...
    public async Task A_Malformed_Datagram_Does_Not_Kill_Reception()
    {
        ...
        SendToMdnsPort([0xFF, 0xFE, 0xFD]);                       // truncated header
        SendToMdnsPort([.. Enumerable.Repeat((byte)0xAA, 400)]);  // structured garbage
        SendToMdnsPort(ValidResponse());
