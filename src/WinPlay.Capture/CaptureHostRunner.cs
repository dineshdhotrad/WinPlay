...
/// <summary>
/// The capture-host child-process entry point. Runs the DXGI + Media Foundation
...
public static class CaptureHostRunner
{
    ...
    public static int Run(string pipeName)
    {
        ...
        try
        {
            ...
            source.FrameEncoded += (au, keyframe, captureTicks) =>
            {
                lock (writeLock)
                {
                    try
                    {
                        ...
                        // Relay the source's own capture-time QPC stamp (shared system timeline) so
                        // the parent can time from capture rather than from pipe-arrival — re-stamping
                        // here would fold this method's own scheduling/lock delay into the "capture"
                        // time, exactly the skew CaptureTicks exists to avoid.
                        MirrorPipeProtocol.WriteFrame(pipe, captureTicks, keyframe, au.Span);
                    }
