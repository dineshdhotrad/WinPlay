...
/// <summary>
/// A crash-isolated <see cref="IH264VideoSource"/>. Instead of capturing and encoding
...
public sealed class SupervisedMirrorSource : IH264VideoSource
{
    ...
    private void Supervise(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            if (failures >= MaxConsecutiveFailuresBeforeFallback)
            {
                ...
                {
                    // The fallback exists because the isolated child kept dying; its own failure
                    // must degrade to a reported end of mirroring, not an unhandled thread
                    // exception at the exact moment the code is trying to be resilient.
                    Diagnostic?.Invoke($"in-process capture failed: {ex.Message}");
                }
                break;
            }
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                switch (msg.Value.Type)
                {
                    ...
                    case MirrorMessageType.Frame:
                        // Relay the child's own capture-time stamp, not "now" — the whole point of
                        // MirrorPipeProtocol carrying CaptureTicks is that pipe transit delay must
                        // not become part of A/V sync error.
                        FrameEncoded?.Invoke(msg.Value.Payload, msg.Value.Keyframe, msg.Value.CaptureTicks);
                        break;
    ...
    private void RunInProcess(CancellationToken ct)
    {
        ...
        src.FrameEncoded += (au, keyframe, captureTicks) => FrameEncoded?.Invoke(au, keyframe, captureTicks);
