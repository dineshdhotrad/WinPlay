...
internal sealed class ProcessLoopbackCapture : IDisposable
{
    ...
    // Remembered so a client killed by an engine-graph rebuild can be recreated exactly as it was.
    private uint _excludeProcessId;
    private WaveFormatEx _format;

    /// <summary>Non-fatal capture events (a client rebuilt, a consumer that threw).</summary>
    ...
    /// False when rebuilding from INSIDE the capture thread after a device invalidation — that
    /// thread is already running and is about to carry on with the new client. Starting another
    /// would leave two threads draining one capture client, each taking half the packets.
    /// </param>
    private void StartExcludingCore(IAudioClient client, WaveFormatEx wfx, bool startThread = true)
    {
    ...
    /// <c>IAudioClient3</c>. <c>GetSharedModeEnginePeriod</c> reports the engine's
    ...
    /// <summary>The granted capture period in milliseconds — the capture-cadence acceptance measure.</summary>
    public double PeriodMilliseconds => PeriodFrames > 0 ? PeriodFrames * 1000.0 / _sampleRate : 20.0;

    private void CaptureLoop()
    {
        // Raise this capture thread to the MMCSS "Pro Audio" class so the scheduler treats
        ...
        try
        {
            while (!_stopped)
            {
                ...
                // Every HRESULT is CHECKED, not just the success case.
                //
                // Windows tears down and rebuilds the audio engine graph when an endpoint idles —
                // routinely, on a laptop, after a stretch of silence — and a shared-mode client
                // bound to the old graph is then answered with AUDCLNT_E_DEVICE_INVALIDATED
                // forever. Treating a non-zero HRESULT as "no packet this time" meant the loop
                // went quietly dead: no samples, no error, no log. Everything downstream is paced
                // by the wall clock rather than by capture, so the pump kept sending, the session
                // stayed connected, the UI still said streaming — and the ring zero-filled silence
                // indefinitely. Pausing music and coming back later put the endpoint through
                // exactly that idle cycle, which is why resuming played nothing until the user
                // toggled the destination off and on: reconnecting was the only path in the whole
                // app that ever built a fresh client.
                int hr = _capture!.GetNextPacketSize(out int frames);
                if (hr != 0)
                {
                    if (!TryRecoverClient(hr)) return;
                    continue;
                }

                while (hr == 0 && frames > 0)
                {
                    if (_capture.GetBuffer(out IntPtr data, out int framesRead, out uint flags, out _, out _) != 0)
                    {
                        if (!TryRecoverClient(hr)) return;
                        break;
                    }
                    ...
                    // A consumer throwing must not kill the capture thread — an unhandled
                    // exception here takes the whole process down by default.
                    try { SamplesAvailable?.Invoke(buffer, framesRead); }
                    catch (Exception ex) { Diagnostic?.Invoke($"capture consumer threw: {ex.Message}"); }

                    hr = _capture.GetNextPacketSize(out frames);
                }
            }
        }
        catch (Exception ex)
        {
            // This thread is process-lifetime and shared by every destination; an escape here is
            // an unhandled thread exception that kills the whole app mid-stream. Report and end
            // the capture instead — the silence detector and session faults take it from there.
    ...
    /// <summary>
    /// Rebuilds the capture client after the engine graph it was bound to went away.
    ///
    /// <para>Tries the cheap repair first — stop, reset, start on the existing client — because
    /// that keeps the negotiated low-latency period. If the client itself is invalid that will
    /// fail too, so it falls back to activating a completely new one. Only when both fail is the
    /// capture genuinely over, and then it says so instead of going silently idle.</para>
    /// </summary>
    private bool TryRecoverClient(int hr)
    {
        if (_stopped) return false;
        Diagnostic?.Invoke($"capture client returned 0x{hr:X8}; rebuilding");
        ...
        try
        {
            ...
            _client?.Reset();
            _client?.Start();
            if (_capture!.GetNextPacketSize(out _) == 0)
            {
                Diagnostic?.Invoke("capture recovered in place");
                return true;
            }
        }
        catch (Exception) { /* the client is gone; fall through to a full rebuild */ }

        try
        {
            // Release the dead one first: an abandoned audio client is not inert, and a later
            // activation can see it as the engine format being held by another app.
            if (_capture is not null) { Marshal.ReleaseComObject(_capture); _capture = null; }
            if (_client is not null) { try { _client.Stop(); } catch (Exception) { } Marshal.ReleaseComObject(_client); _client = null; }

            _client = Activate(_excludeProcessId, mode: 1);
            StartExcludingCore(_client, _format, startThread: false);
            Diagnostic?.Invoke("capture client rebuilt");
            return true;
        }
        catch (Exception ex)
        {
            Diagnostic?.Invoke($"capture could not be rebuilt: {ex.Message}");
            CaptureFailed?.Invoke(ex);
            return false;
        }
    }

    /// <summary>Capture has stopped for good and cannot be rebuilt.</summary>
    ...
    private static IAudioClient Activate(uint targetPid, uint mode)
    {
        ...
        try
        {
            ...
            // low-latency path: the object returned for a plain IAudioClient request on
