...
/// HomePod or Apple TV control playback on the PC.
///
...
public sealed class DacpServer : IDisposable
{
    ...
    /// <summary>
    /// A receiver issued a transport command: (origin from <see cref="IssueToken"/>, command).
    /// Transport addresses the PC's single media session, so most handlers ignore the origin — but
    /// the volume keys arrive here too, and those mean "this speaker".
    /// </summary>
    public event Action<string, DacpCommand>? CommandReceived;

    /// <summary>
    /// Tokens handed to individual destinations, mapped back to a caller-chosen origin name.
    /// Keyed by token so an inbound request identifies its sender.
    /// </summary>
    private readonly System.Collections.Concurrent.ConcurrentDictionary<string, string> _origins = new(StringComparer.Ordinal);

    /// <summary>
    /// Issues an <c>Active-Remote</c> token that identifies one destination.
    ///
    /// <para>Transport commands (play, pause, next) address the PC's single media session, so one
    /// shared identity is right for them. VOLUME is not like that: turning the dial on a HomePod
    /// means "make THIS speaker quieter", and with one token for everything the app could not tell
    /// which speaker asked — so it changed them all, including an unrelated room the user was
    /// streaming to independently. A distinct token per destination makes the sender identifiable
    /// without needing a second service or a second port: the DACP-ID, and therefore the DNS-SD
    /// instance receivers resolve, stays shared.</para>
    /// </summary>
    public string IssueToken(string origin)
    {
        string token = RandomNumberGenerator.GetInt32(1, int.MaxValue).ToString();
        _origins[token] = origin;
        return token;
    }

    /// <summary>Retires a destination's token when its session ends.</summary>
    public void RevokeToken(string token)
    {
        if (token is not null) _origins.TryRemove(token, out _);
    }

    /// <summary>
    /// Resolves an echoed token to the destination that was given it, or null if it matches
    /// nothing. Every candidate is compared in constant time and the scan always runs to
    /// completion, so neither the answer nor the timing leaks a valid token.
    /// </summary>
    private string? MatchToken(string? presented)
    {
        byte[] candidate = Encoding.UTF8.GetBytes(presented ?? "");
        string? origin = null;

        // The shared identity still answers, for receivers set up before any per-destination
        // token existed and for transport control in general.
        if (CryptographicOperations.FixedTimeEquals(candidate, Encoding.UTF8.GetBytes(ActiveRemote)))
            origin = "this PC";

        foreach (var (token, name) in _origins)
        {
            if (CryptographicOperations.FixedTimeEquals(candidate, Encoding.UTF8.GetBytes(token)))
                origin = name;
        }
        return origin;
    }
    ...
    /// <summary>A receiver asked for a volume change: (origin from <see cref="IssueToken"/>, dB).</summary>
    public event Action<string, double>? VolumeRequested;
    ...
    private async Task ServeAsync(TcpClient client, CancellationToken ct)
    {
        using (client)
        {
            try
            {
                ...
                // Which sender-side token was echoed identifies WHICH receiver is asking. Every
                // token is compared in constant time, and the loop deliberately does not stop at
                // the first match, so the work does not depend on the secret.
                string? origin = MatchToken(request.ActiveRemote);
                if (origin is null)
                {
                ...
                {
                    Diagnostic?.Invoke($"DACP volume request from {origin}: {db:F1} dB");
                    VolumeRequested?.Invoke(origin, db);
                }
                else if (request.Command != DacpCommand.Unknown)
                {
                    Diagnostic?.Invoke($"DACP command from {origin}: {request.Command}");
                    CommandReceived?.Invoke(origin, request.Command);
                }
            }
