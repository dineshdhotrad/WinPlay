...
public sealed class PickerRowViewModel : INotifyPropertyChanged
{
    ...
    {
        _audioAttempt?.Cancel();          // the superseded attempt disposes its own source
    ...
    public bool ShowVolume => _isAudioChecked && !_isAudioBusy;
    ...
    {
        if (!IsBusy) return; // only surface progress while connecting
