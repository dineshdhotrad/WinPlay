...
public sealed class MainViewModel : INotifyPropertyChanged, IAsyncDisposable
{
    ...
    private async Task OnMirrorToggleAsync(PickerRowViewModel row, bool on)
    {
        ...
        try
        {
            if (on)
            {
                ...
                await _streams.StartMirrorAsync(row.Entry, attempt.Token);
                _dispatcher.TryEnqueue(() =>
                {
                    if (!row.IsCurrent(attempt)) return;
    ...
    private bool TryStartDiscovery()
    {
        try
        {
            ...
            // WinPlay's own DACP control endpoint so receivers can send play/pause back (D3).
    ...
    /// (H2). Names/models/addresses make a bug actionable; the bundle redacts key material.
    /// </summary>
    ...
    public string NowPlayingTitle
    {
        ...
        private set { _nowPlayingTitle = value; OnPropertyChanged(); OnPropertyChanged(nameof(HasNowPlaying)); }
    }
