...
public sealed class StreamController : IAsyncDisposable
{
    ...
    public async Task StartAudioAsync(PickerEntry entry, double volumeDb, CancellationToken ct)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var session = await ConnectAudioAsync(entry, members, dest.Cts.Token, dest.RemoteToken).ConfigureAwait(false);
        ...
        IReadOnlyList<GroupSession.Member> members, CancellationToken ct, string? activeRemote = null)
    {
        try
        {
            return await ConnectAsync(entry, members, ct, activeRemote).ConfigureAwait(false);
        }
        catch (AggregateException ex) when (ex.InnerExceptions.OfType<StalePairingException>().Any())
        {
            ...
            return await ConnectAsync(entry, ResolveMembers(entry), ct, activeRemote).ConfigureAwait(false);
        }
    ...
    public double VolumeOf(string key) =>
        _audio.TryGetValue(key, out var dest) ? dest.VolumeDb : VolumeControl.MaxDb;
        ...
        IReadOnlyList<GroupSession.Member> members, CancellationToken ct, string? activeRemote = null)
    {
        ...
        try
        {
            ...
            return await GroupSession.ConnectAsync(members,
                ...
                timeout.Token, buffered: true, identities: _identities,
                activeRemote: activeRemote).ConfigureAwait(false);
        }
