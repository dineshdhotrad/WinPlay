...
public sealed class NowPlayingService : IAsyncDisposable
{
    ...
    /// the picker's Now Playing surface (F1); raised whether or not anything is streaming.</summary>
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                if (session is not null)
                {
                    ...
                    // advances continuously even when the track does not change (D2).
