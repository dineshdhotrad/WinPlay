...
public sealed class RemoteControlService : IAsyncDisposable
{
    ...
    /// </summary>
    private void OnVolumeRequested(string origin, double db) => _ = ApplyVolumeAsync(origin, db);

    private Task ApplyVolumeAsync(string origin, double db) =>
        _streams.IsAudioActive(origin) ? _streams.SetVolumeAsync(origin, db) : _streams.SetAllVolumesAsync(db);
