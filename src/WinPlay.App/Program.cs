...
/// activation redirection (A3). A second launch registers against the same key, discovers
...
public static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        // to that child process (A4). Handle it before any WinUI / single-instance init.
        if (args.Length >= 2 && args[0] == CaptureHostRunner.Switch)
            return CaptureHostRunner.Run(args[1]);

        // Log verbosity from the command line (H1). Read before App is constructed so the very
    ...
    private static void RedirectActivationTo(AppActivationArguments args, AppInstance primary)
    {
        ...
        Task.Run(() =>
        {
            ...
            SetEvent(_redirectEvent);
        });
        const uint infinite = 0xFFFFFFFF;
        _ = CoWaitForMultipleObjects(0, infinite, 1, [_redirectEvent], out _);
    }
